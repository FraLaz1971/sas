...
class TDSSourceDaemon : public Daemon  {
public:   
    TDSSourceDaemon();
    TDSSourceDaemon(vector<string> dbc);
    ~TDSSourceDaemon();
    void start(double tstart, double tstep);
    void setCotDir(string cotd);
    string getCotDir();
    void cotProcess();
    void launch_TDS_Source(double mtstart, double mtstop, double msrc_ra, double msrc_dec, string prefix);
