/***************************************************************************
                          tdssourcedaemon.cpp  -  description
                             -------------------
...
#include "tdssourcedaemon.h"
namespace SApipeline {
  namespace TDS {
TDSSourceDaemon::TDSSourceDaemon(){
...
TDSSourceDaemon::TDSSourceDaemon(vector<string> dbc){
tempstring = "/home/agile/projects/AUX/gtbweb.bo.iasf.cnr.it/private/gtb_data/AGILEGS/PREPROCESSING/COMMISS_001/temp_arch/AUX";
	setCotDir(tempstring);
...
/*!
    \fn SApipeline::TDS::TDSSourceDaemon::start()
 */
void SApipeline::TDS::TDSSourceDaemon::start(double tstart, double tstep)
{
...
/*!
    \fn SApipeline::TDS::TDSSourceDaemon::setCotDir(string cotd)
 */
void SApipeline::TDS::TDSSourceDaemon::setCotDir(string cotd)
{
    cotdir = cotd;
}
...
/*!
    \fn SApipeline::TDS::TDSSourceDaemon::getCotDir()
 */
string SApipeline::TDS::TDSSourceDaemon::getCotDir()
{
   return cotdir;
}
...
/*!
    \fn SApipeline::TDS::TDSSourceDaemon::cotProcess()
 */
void SApipeline::TDS::TDSSourceDaemon::cotProcess()
{
/*
...
/*!
    \fn SApipeline::TDS::TDSSourceDaemon::launch_TDS_Source(double mtstart, double mtstop, double src_ra, double src_dec, string prefix)
 */
void SApipeline::TDS::TDSSourceDaemon::launch_TDS_Source(double mtstart, double mtstop, double msrc_ra, double msrc_dec, string prefix)
{
...
// ################################
	...
	cout << "TDSSourceDaemon::launch_TDS_Source() going on !!! "<< endl;
    ...
    dbp->loadDBDefaults();
    se.setMsg("TDSSourceDaemon::launch_TDS_Source() error in connecting to the database ");
	...
	cout << "TDSSourceDaemon::launch_TDS_Source() first value is " << contactmin << " last value is " << contactmax << endl;

	se.setMsg("TDSSourceDaemon::launch_TDS_Source() error in disconnecting the database ");
 ...
 cerr << "error " << se.getMsg() << endl;
   ...
   cerr << "error " <<  endl;
		...
		cout << "TDSSourceDaemon::launch_TDS_Source(): downloading files ..." << endl;
		...
		cout << "TDSSourceDaemon::launch_TDS_Source(): downloaded telemetry data  ..." << endl;

		ss.str("");ss << "./rsync_l2file.sh " << getLocalHost() << " " << getRemoteHost() << " " << getRemoteUser() << " " << contint[0] << " " << contint[1] << endl;
		tempstring = ss.str();syscon.execCommand(tempstring);

		cout << "TDSSourceDaemon::launch_TDS_Source(): downloaded l2 data  ..." << endl;
		cout << "TDSSourceDaemon::launch_TDS_Source() echo unzipping files ..." << endl;
		cout << "TDSSourceDaemon::launch_TDS_Source() testing translating functions from " << endl;
		cout << "TDSSourceDaemon::launch_TDS_Source() centint of tstart = " << tdbuilder.getCentString(contint[0])<< endl;
		cout << "TDSSourceDaemon::launch_TDS_Source() centint of tstop = " << tdbuilder.getCentString(contint[1])<< endl;
		...
		tempstring = ss.str();
		if (verbose) cout << "TDSSourceDaemon::launch_TDS_Source() executing " << tempstring << endl;
		syscon.execCommand(tempstring);
	...
	try{
		...
		if (verbose) cout << "TDSSourceDaemon::launch_TDS_Source() removing local files !"<< endl;
...
/*!
    \fn SApipeline::TDS::TDSSourceDaemon::getContactRange(double mtstart, double mtstop)
 */
vector<int> SApipeline::TDS::TDSSourceDaemon::getContactRange(double mtstart, double mtstop)
{
  tstart = mtstart; tstop = mtstop; 
...
//  string gf = "select contactnumber from ContactTable where (tstop > \"2008-02-05 17:00:33\") AND (tstop < \"2008-02-05 22:42:48\") ";
  ...
  if (verbose) cout << "TDSSourceDaemon::getContactRange() executing query on DB: " << gf << endl;
  string tempstring;
...
//  vector<string>::iterator svi = sv.begin();
  try{
	...
	if (verbose ) cout << "TDSSourceDaemon::getContactRange() first value is " << sv[0] << "TDSSourceDaemon::getContactRange() last value is " << sv[sv.size() - 1]<< endl;
...
/*!
    \fn SApipeline::TDS::TDSSourceDaemon::process()
 */
int SApipeline::TDS::TDSSourceDaemon::process()
{
...
*/
 ...
 if (verbose) cout << "TDSSourceDaemon::process() here 3" << endl;
  sh_varname = "SASOA_WORKDIR"; 
  try{
   sh_value = mysystem.getEnvVar(sh_varname);
  } catch (const SAexception &se){
    cerr << "TDSSourceDaemon::process() : errror in getting SASOA_WORKDIR env var" << se.getMsg() << endl;
  } catch (const exception &e){
    cerr << "TDSSourceDaemon::process() : errror in getting SASOA_WORKDIR env var" << e.what() << endl;
  } catch (...){
    cerr << "TDSSourceDaemon::process() : errror in getting SASOA_WORKDIR env var" << endl;
  }
  ...
  if (verbose) cout << "TDSSourceDaemon::process() executing "<< dc << endl;

  try{
  system(dc.c_str());
  } catch (const SAexception &se){
    cerr << "TDSSourceDaemon::process() : errror in removing deliverable data" << se.getMsg() << endl;
  } catch (const exception &e){
    cerr << "TDSSourceDaemon::process() : errror in removing deliverable data" << e.what() << endl;
  } catch (...){
    cerr << "TDSSourceDaemon::process() : errror in removing deliverable data" << endl;
  }
...
//---------> 
  ...
  if (verbose) cout << "TDSSourceDaemon::process() launching attitude correction " << endl;

  try{
  system(bb.c_str());
  } catch (const SAexception &se){
    cerr << "TDSSourceDaemon::process() : errror in attitude correction" << se.getMsg() << endl;
  } catch (const exception &e){
    cerr << "TDSSourceDaemon::process() : errror in attitude correction" << e.what() << endl;
  } catch (...){
    cerr << "TDSSourceDaemon::process() : errror in attitude correction" << endl;
  }

  //string tempstring =   srcx.getUserHome()+"/SASOA/"+srcx.getDetnam()+"/data/L3";

  // copy L3 file (attitude corrected eventlist) in the $SASOA_WORKDIR/SA/delivery/
  //directory
  cout << "TDSSourceDaemon::process() executing " << tc << endl;

try{
  system(tc.c_str());
  } catch (const SAexception &se){
    cerr << "TDSSourceDaemon::process() : errror in copying attitude corrected eventlist" << se.getMsg() << endl;
  } catch (const exception &e){
    cerr << "TDSSourceDaemon::process() : errror in copying attitude corrected eventlist" << e.what() << endl;
  } catch (...){
    cerr << "TDSSourceDaemon::process() : errror in copying attitude corrected eventlist" << endl;
  }
...
// if (argc<8) cout << "usage: ./runTDS_SRC <exp_datacard_out_filename> <sapoifilename> <ephfilename> <tstart> <tstop> <contactmin> <contactmax> <src_ra> <src_dec> <src_prefix>" << endl;
  ...
  if (verbose) cout << "TDSSourceDaemon::process() using ephemeris file " << ephfile << " for earth occultation filtering" << endl;

 //// Inserire EXPOSURE_PROFILE
  ...
  if (verbose) cout << "TDSSourceDaemon::process() executing " << L3earthFilter << endl;
...
/*!
    \fn SApipeline::TDS::TDSSourceDaemon::processSRC()
 */
void SApipeline::TDS::TDSSourceDaemon::processSRC()
{
  ...
  try{
   sh_value = mysystem.getEnvVar(sh_varname);
    ...
    cerr << "TDSSourceDaemon::process() : errror in getting SASOA_HOME env var" << se.getMsg() << endl;
  } catch (const exception &e){
    cerr << "TDSSourceDaemon::process() : errror in getting SASOA_HOME env var" << e.what() << endl;
  } catch (...){
    cerr << "TDSSourceDaemon::process() : errror in getting SASOA_HOME env var" << endl;
  }
...
/*!
    \fn SApipeline::TDS::TDSSourceDaemon::initArchiveEnv()
 */
void SApipeline::TDS::TDSSourceDaemon::initArchiveEnv(ArchiveHandler *sarchiver, PipelineModule &pipemodule)
{
try{
  string temp;
  ...
  sarchiver->setDataDirs();
	...
	cerr << "TDSSourceDaemon::initArchiveEnv() error in initializing archiver environment "<< se.getMsg()<< endl;
}
catch(const exception & e){
	cerr << "TDSSourceDaemon::initArchiveEnv() error in initializing archiver environment "<< e.what()<< endl;
...
/*!
    \fn SApipeline::TDS::TDSSourceDaemon::setLocalHost(string lh)
 */
void SApipeline::TDS::TDSSourceDaemon::setLocalHost(string lh)
{
    localhost = lh;
}

/*!
    \fn SApipeline::TDS::TDSSourceDaemon::getLocalHost()
 */
string SApipeline::TDS::TDSSourceDaemon::getLocalHost()
{
    return localhost;
}
...
/*!
    \fn SApipeline::TDS::TDSSourceDaemon::setRemoteUser()
 */
void SApipeline::TDS::TDSSourceDaemon::setRemoteUser(string ru)
{
    remoteuser = ru;
}
...
/*!
    \fn SApipeline::TDS::TDSSourceDaemon::setRemoteHost()
 */
void SApipeline::TDS::TDSSourceDaemon::setRemoteHost(string rh)
{
    remotehost = rh;
}
...
/*!
    \fn SApipeline::TDS::TDSSourceDaemon::getRemoteHost()
 */
string SApipeline::TDS::TDSSourceDaemon::getRemoteHost()
{
    return remotehost;
}
...
/*!
    \fn SApipeline::TDS::TDSSourceDaemon::getRemoteUser()
 */
string SApipeline::TDS::TDSSourceDaemon::getRemoteUser()
{
    return remoteuser;
}
